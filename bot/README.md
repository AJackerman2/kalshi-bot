...
bot/                            (you are here)
  src/kalshi_maker_bot/
    ...
    events.py            EventBus: DB + JSONL + Sheets + Postgres fan-out
...
`supabase/migrations/` lives at the repo root and holds the Postgres
schema shared with the dashboard.  The dashboard itself is at the repo
root (see `../README.md`).
...
bash bot/scripts/install.sh
# place RSA key at /etc/kalshi-maker-bot/kalshi_private_key.pem
# (optional) place Google JSON at /etc/kalshi-maker-bot/google-credentials.json
# edit /etc/kalshi-maker-bot/.env (keep MODE=sim; fill PG_DATABASE_URL)
